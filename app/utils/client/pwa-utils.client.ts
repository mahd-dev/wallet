 ...
 * @return {Promise<ResponseObject>} An object consisting of two properties: A status to indicate the status of the invocation and also an accompanying message.
 */
export async function setBadge(numberCount: number): Promise<ResponseObject> {
  try {
    if (navigator.setAppBadge) {
      await navigator.setAppBadge(numberCount);
      return {
        status: "success",
        message: "Badge successfully set",
      };
    } else {
      return {
        status: "bad",
    ...
    console.debug(err);
    throw new Error("Error setting badge!");
  }
}
...
export async function WebShareLink(
  ...
  if (!navigator.canShare({ url, text, title })) {
    throw "Web Share API not supported";
  }
...
export function arraybuffer2base64String(arraybuffer: ArrayBuffer) {
  let binary = "";
...
// Subscribe to push service and return device's push data in base64 format.

/**
 * Subscribe to push service and return device's push data in base64 format.
 *
 * @return {Promise<String>} The base64 format device data to use to send notification.
 */
export async function pushSubscribe(request: boolean): Promise<string> {
  ...
  const sub = await sw.pushManager.subscribe({
    ...
    applicationServerKey: urlBase64ToUint8Array(
      (await (await fetch("/api/push/pubkey")).json()).key,
    ),
  });
  ...
  const device = btoa(
    JSON.stringify({
      endpoint,
      expirationTime,
      key: arraybuffer2base64String(key),
      auth: arraybuffer2base64String(auth),
    }),
  );
